...
export function StudyManagerView() {
  ...
  const { state: isCollapsed, setState: setIsCollapsed } = useLocalStorage(
    'study-manager-sidebar-collapsed',
    false
  );
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      if (conceptIdx !== null && !isNaN(parseInt(conceptIdx, 10))) {
        setActiveConceptIndex(parseInt(conceptIdx, 10));
      } else {
  ...
  const updateURL = useCallback(
    (params: {
      ...
      if (params.conceptIndex !== undefined) {
        if (params.conceptIndex !== null && params.conceptIndex !== undefined) {
          newParams.set('conceptIndex', String(params.conceptIndex));
        } else {
          newParams.delete('conceptIndex');
        }
      }
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id) {
        ...
        const targetFolderId = targetId === 'study_root' || targetId === 'root' ? null : targetId;
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await getStudyTreeData();
        const sanitizedData = sanitizeTreeData(data);
        setTreeData(sanitizedData);
      } catch (error) {
        console.error('Failed to load study tree data from IndexedDB', error);
      } finally {
        setIsLoaded(true);
      }
    };
  ...
  useEffect(() => {
    if (isLoaded) {
      saveStudyTreeData(treeData).catch((error) => {
        console.error('Failed to save study tree data to IndexedDB', error);
      });
    }
  }, [treeData, isLoaded]);
  ...
  const handleDownload = useCallback(async () => {
    const fullData = await getStudyFullData();
    ...
    link.download = `study-backup-${date}-${time}.json`;
  ...
  const handleReset = useCallback(async () => {
    ...
    const data = await getStudyTreeData();
    setTreeData(sanitizeTreeData(data));
    handleNavigate(null);
    toast.success('Study reset success (study.json)!');
  }, [handleNavigate]);

  const applyUpload = useCallback(async (data: any) => {
    if (data && typeof data === 'object' && 'tree' in data && 'scripts' in data) {
      await saveStudyFullData(data);
      setTreeData(sanitizeTreeData(data.tree));
      toast.success('Upload success!');
    } else if (Array.isArray(data)) {
      await saveStudyFullData({ tree: data, scripts: {} });
      setTreeData(sanitizeTreeData(data));
  ...
  const executeDelete = useCallback(
    async (ids: string[]) => {
      try {
        await deleteStudyTreeItems(ids);
        const data = await getStudyTreeData();
        const sanitizedData = sanitizeTreeData(data);
        setTreeData(sanitizedData);
  ...
  const handleCopyItem = useCallback((id: string) => {
    ...
    const duplicateNode = (node: any): any => {
      ...
      return {
        ...
        isFavorited: false,
        ...(node.children && {
          children: node.children.map((child: any) => duplicateNode(child)),
        }),
      };
    };

    const findAndCopy = (nodes: any[]): any[] => {
      const result: any[] = [];
      for (const node of nodes) {
        result.push(node);
        if (node.id === id) {
          copiedNode = duplicateNode(node);
    ...
    setTreeData((prev) => {
      const newNodes = findAndCopy(prev);
      if (copiedNode) {
          ...
          >
            {viewMode === 'list' ? (
              ...
              <StudyDocEditorView
                ...
                initialConceptIndex={activeConceptIndex}
                onBack={() => updateURL({ view: 'live', conceptIndex: activeConceptIndex })}
                onSaveSuccess={(savedIdx) => {
                  const targetIdx = savedIdx ?? activeConceptIndex;
              ...
              <StudyDocView
                ...
                initialConceptIndex={activeConceptIndex}
                onBack={() =>
                  updateURL({ view: 'list', fileId: null, fileName: null, conceptIndex: null })
                }
                onEdit={(editIdx) => {
                  const targetIdx = editIdx ?? activeConceptIndex;
                  setActiveConceptIndex(targetIdx);
                  updateURL({ view: 'editor', conceptIndex: targetIdx });
                }}
              />
            ) : null}
          </Box>
